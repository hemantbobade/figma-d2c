...
### Multi-Screen Flow File Structure

When a Figma conversion contains **2 or more distinct screens or steps** (e.g. onboarding, checkout, multi-step forms), **always split into one file per screen**. Never place multiple screens in a single file.

**Required directory structure:**

```
src/<feature>/
├── <Feature>.tsx          ← Orchestrator only: step state + navigation (~50–80 lines max)
├── <feature>.css          ← Shared custom CSS
├── types.ts               ← Shared interfaces, form-data types, constants
├── shared/
│   ├── <Feature>Header.tsx
│   ├── <Feature>Footer.tsx
│   └── <SharedComponent>.tsx   ← stepper, progress bar, etc.
└── screens/
    ├── <ScreenName>.tsx   ← one file per distinct screen/step
    └── ...
```

**Rules — enforced on every multi-screen conversion:**

- `types.ts` exports all shared TypeScript interfaces, form-data type, shared constants (step labels, totals, etc.)
- `<Feature>.tsx` (orchestrator) contains **only**: `useState`, navigation handlers (`goNext`, `goBack`, `goToStep`), and a single conditional render block. **No JSX layout.**
- Each screen file exports exactly one named component.
- Shared chrome (header, footer, stepper) each gets its own file in `shared/`.
- Screen components import from `../types` and `../shared/`; they never import sibling screens.

**❌ Wrong — all screens in one file:**

```tsx
// Onboarding.tsx  (~1100 lines) — BAD
export function Onboarding() { ... }
function WelcomeScreen() { ... }     // ← belongs in screens/WelcomeScreen.tsx
function PersonalInfoStep() { ... }  // ← belongs in screens/PersonalInfoStep.tsx
```

**✅ Correct — orchestrator delegates to screen files:**

```tsx
// Onboarding.tsx — orchestrator only (~60 lines)
...
### ❌ Pitfall 4: Custom div wrappers for section areas and cards

```tsx
// WRONG — custom divs with manual CSS for padding, background, border
<div className="my-hero">           {/* background + padding */}
  ...
</div>
<div className="my-feature-card">  {/* border + radius + padding */}
  ...
</div>
```

**Fix**: Use `Section` for background/padding zones and `Card` for bordered content boxes:

```tsx
// Section variants: "subtle" (#fafafa), "neutral" (tertiary gray), "stroke" (border), "brand"
<Section padding="1600" variant="subtle">  {/* replaces hero/content wrappers */}
  ...
</Section>

// Card variants: "stroke" (border + shadow), "brand", "default"
// Card padding: "600" or "800"
import { Card } from "compositions";
<Card variant="stroke" padding="600">      {/* replaces any bordered card div */}
  ...
</Card>
```

**Known limits** — these custom CSS patterns are irreducible and are acceptable exceptions:
- `Section` only applies **top/bottom** padding (left/right is always `space-600`). If a section needs wider horizontal padding, a wrapping div with custom horizontal padding is required.
- `Flex container` has a single global max-width. If you need varying max-widths (e.g. 720px narrow, 1180px wide), a custom `.inner` class with `margin: 0 auto` and `max-width` is required.
- `Card` has no `box-shadow` prop. If a shadow beyond the built-in stroke variant is needed, a custom CSS class is required.
- Brand-specific hex colors with no SDS token (e.g. `#0d6efd`, `#e7f0ff`) must stay in custom CSS with an inline comment documenting why no token exists.
